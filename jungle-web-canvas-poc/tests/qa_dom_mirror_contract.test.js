...
test("qa=1 exposes a mutation-driven input command DOM contract without synthetic Event constructors", () => {
  assert.match(INPUT, /qa-input-command/);
  assert.match(INPUT, /eduni-jungle-input-v1/);
  assert.match(INPUT, /new MutationObserver\(applyCommand\)/);
  assert.match(INPUT, /attributeFilter: \["data-command"\]/);
  assert.match(INPUT, /this\.setDigitalAction\(action, state === "down"\)/);
  assert.match(INPUT, /el\.dataset\.lastApplied = raw/);
  assert.match(INPUT, /el\.dataset\.direction = JSON\.stringify\(this\.direction\(\)\)/);
});
