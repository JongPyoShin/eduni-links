...
_DIRECTION_IMPLEMENTATION = r'''      function directionAnswerKey(row,col) {
        return `${row},${col}`;
      }
      ...
      function buildDirectionChoicePositions(scenario) {
        const correct=[scenario.finalRow,scenario.finalCol];
        ...
        for (const [dr,dc] of positionOffsets) {
          ...
          const key=directionAnswerKey(r,c);
      ...
      function auditDirectionUiContracts() {
        ...
        scenarios.forEach((scenario,index) => {
          ...
          // Internal scenario integrity still checks the final facing direction,
          // even though the child-facing answer asks only for the destination cell.
          ...
          const positions=buildDirectionChoicePositions(scenario);
          const keys=positions.map(([r,c]) => directionAnswerKey(r,c));
          const correctKey=directionAnswerKey(scenario.finalRow,scenario.finalCol);
          if (positions.length!==3 || new Set(keys).size!==3) failures.push(`${id}: three unique destination choices required`);
          if (keys.filter((key) => key===correctKey).length!==1) failures.push(`${id}: correct destination must appear exactly once`);
          positions.forEach(([r,c],choiceIndex) => {
            const rendered=renderDirectionGrid(r,c,null,true);
            if (!rendered.includes('★')) failures.push(`${id}: choice ${choiceIndex+1} does not render destination star`);
            if (DIRECTION_ARROWS.some((arrow) => rendered.includes(arrow))) failures.push(`${id}: choice ${choiceIndex+1} must not render a facing arrow`);
          });
        });
        ...
        if (status) {
          if (report.ok && bankAudit && bankAudit.ok===true) {
            status.textContent='✅ 문제은행 470/470 + 방향 도착칸 50/50 검증 통과';
        ...
        if (!report.ok) {
          console.error('EDUNI direction destination UI audit failed',report);
          throw new Error(`EDUNI direction destination UI audit failed (${failures.length}): ${failures.slice(0,5).join(' | ')}`);
        }
        return report;
      }
      ...
      function makeDirectionQuestion() {
        ...
        const correctKey=directionAnswerKey(scenario.finalRow,scenario.finalCol);
        const positions=buildDirectionChoicePositions(scenario);
        if (positions.length!==3) throw new Error('EDUNI direction question could not build 3 unique destination choices');
        const options=shuffle(positions).map(([r,c]) => ({
          key:directionAnswerKey(r,c),
          html:renderDirectionGrid(r,c,null,true),
        }));
        const optionKeys=options.map((option) => option.key);
        if (new Set(optionKeys).size!==3 || optionKeys.filter((key) => key===correctKey).length!==1) {
          throw new Error('EDUNI direction destination choices failed single-answer contract');
        }
        ...
        return {
          ...
          options,
          correctKey,
          hint:'돌기는 제자리에서 방향만 바뀌어요. 방향을 바꾼 뒤 앞으로 이동한 칸을 차례대로 따라가고, 마지막 별의 위치만 골라봐.',
        };
      }
...
def _fix_destination_marker(html: str) -> str:
    ...
    new = (
        "          body += `<circle cx=\"${x}\" cy=\"${y}\" r=\"13\" fill=\"#7c3aed\"/>"
        "<text x=\"${x}\" y=\"${y+6}\" text-anchor=\"middle\" font-size=\"17\" font-weight=\"1000\" fill=\"#fff\">★</text>`;"
    )
    if old not in html:
